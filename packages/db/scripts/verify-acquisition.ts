...
const organization = await createAnalyticsOrganization({
  name: `Acquisition QA ${suffix}`,
  slug: `acquisition-qa-${suffix}`,
});
const project = `acquisition-qa-${suffix}`;
...
for (let offset = 0; offset < events.length; offset += 25) {
  ...
  await ingestEventBatch(batch, "local-acquisition-qa-only");
}
const query = {
  project,
...
console.log(
  `Acquisition SQL checks passed. Retained local QA fixture: /insights?organization=${organization.slug}&project=${project}`,
);
process.exit(0);
